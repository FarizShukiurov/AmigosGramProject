...
import {
    ...
    StopOutlined,
    CameraOutlined,
    PaperClipOutlined, // иконка скрепки
    SmileOutlined,
    SendOutlined,
} from "@ant-design/icons";
import Picker from "emoji-picker-react";
import "./ChatPageMobile.css"; // Здесь можно определить стили и анимации для мобильной версии
...
function ChatPageMobile() {
    ...
    // Пример обработчика возврата из чата к списку
    const handleBack = () => {
        setSelectedChatId(null);
    };
    return (
        <Layout className="chat-page mobile">
            {/* Ограничиваем горизонтальный скроллинг */}
            <div className="mobile-container">
                {!selectedChatId ? (
                    <div className="chat-list-view animated-slide-in">
                    ...
                    <div className="chat-detail-view animated-slide-in">
                            <div className="chat-header">
                                <Button icon={<ArrowLeftOutlined />} onClick={handleBack} />
                                {currentChat && (
                                    <>
                                        <Avatar src={currentChat.avatarUrl} size={40} style={{ marginRight: '8px' }} />
                                        <span className="chat-title">{currentChat.userName}</span>
                                    </>
                                )}
                            </div>
